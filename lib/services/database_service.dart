// import 'package:chat_app/models/chat.dart';
// import 'package:chat_app/models/message.dart';
// import 'package:chat_app/models/user_profile.dart';
// import 'package:chat_app/services/auth_service.dart';
// import 'package:chat_app/utils.dart';
// import 'package:cloud_firestore/cloud_firestore.dart';
// import 'package:get_it/get_it.dart';

// class DatabaseService {
//   final GetIt _getIt = GetIt.instance;

//   final FirebaseFirestore _firebaseFirestore = FirebaseFirestore.instance;
//   late AuthService _authService;
//   CollectionReference? _usersCollection;
//   CollectionReference? _chatsCollection;

//   DatabaseService() {
//     _authService = _getIt.get<AuthService>();

//     _setupCollectionReferances();
//   }

//   void _setupCollectionReferances() {
//     _usersCollection = _firebaseFirestore
//         .collection('users')
//         .withConverter<UserProfile>(
//             fromFirestore: (snapshot, _) =>
//                 UserProfile.fromJson(snapshot.data()!),
//             toFirestore: (userProfile, _) => userProfile.toJson());
//     _chatsCollection = _firebaseFirestore
//         .collection("chats")
//         .withConverter<Chat>(
//             fromFirestore: (snapshot, _) => Chat.fromJson(snapshot.data()!),
//             toFirestore: (chat, _) => chat.toJson());
//   }

//   Future<void> createUserProfile({required UserProfile userProfile}) async {
//     await _usersCollection?.doc(userProfile.uid).set(userProfile);
//   }

//   Stream<QuerySnapshot<UserProfile>> getUserProfiles() {
//     return _usersCollection
//         ?.where("uid", isNotEqualTo: _authService.user!.uid)
//         .snapshots() as Stream<QuerySnapshot<UserProfile>>;
//   }

//   Future<bool> checkChatExists(
//     String uid1,
//     String uid2,
//   ) async {
//     String chatID = generateChatID(uid1: uid1, uid2: uid2);
//     final result = await _chatsCollection?.doc(chatID).get();
//     if (result != null) {
//       return result.exists;
//     }
//     return false;
//   }

//   Future<void> createNewChat(
//     String uid1,
//     String uid2,
//   ) async {
//     String chatID = generateChatID(uid1: uid1, uid2: uid2);
//     final docRef = _chatsCollection!.doc(chatID);
//     final chat = Chat(
//       id: chatID,
//       participants: [uid1, uid2],
//       messages: [],
//     );
//     await docRef.set(chat);
//   }

//   Future<void> sendChatMessage(
//       String uid1, String uid2, Message message) async {
//     String chatID = generateChatID(uid1: uid1, uid2: uid2);
//     final docRef = _chatsCollection!.doc(chatID);
//     await docRef.update(
//       {
//         "messages": FieldValue.arrayUnion(
//           [
//             message.toJson(),
//           ],
//         ),
//       },
//     );
//   }

//   Stream<DocumentSnapshot<Chat>> getChatData(String uid1, String uid2) {
//     String chatID = generateChatID(uid1: uid1, uid2: uid2);
//     return _chatsCollection?.doc(chatID).snapshots()
//         as Stream<DocumentSnapshot<Chat>>;
//   }
// }

import 'package:chat_app/models/chat.dart';
import 'package:chat_app/models/message.dart';
import 'package:chat_app/models/user_profile.dart';
import 'package:chat_app/services/auth_service.dart';
import 'package:chat_app/utils.dart';
import 'package:cloud_firestore/cloud_firestore.dart';
import 'package:get_it/get_it.dart';

class DatabaseService {
  final GetIt _getIt = GetIt.instance;
  final FirebaseFirestore _firebaseFirestore = FirebaseFirestore.instance;
  late AuthService _authService;
  CollectionReference<UserProfile>? _usersCollection;
  CollectionReference<Chat>? _chatsCollection;

  DatabaseService() {
    _authService = _getIt.get<AuthService>();
    _setupCollectionReferences();
  }

  void _setupCollectionReferences() {
    _usersCollection =
        _firebaseFirestore.collection('users').withConverter<UserProfile>(
              fromFirestore: (snapshot, _) =>
                  UserProfile.fromJson(snapshot.data()!),
              toFirestore: (userProfile, _) => userProfile.toJson(),
            );

    _chatsCollection =
        _firebaseFirestore.collection("chats").withConverter<Chat>(
              fromFirestore: (snapshot, _) => Chat.fromJson(snapshot.data()!),
              toFirestore: (chat, _) => chat.toJson(),
            );
  }

  Future<void> createUserProfile({required UserProfile userProfile}) async {
    await _usersCollection?.doc(userProfile.uid).set(userProfile);
  }

  Stream<QuerySnapshot<UserProfile>> getUserProfiles() {
    return _usersCollection!
        .where("uid", isNotEqualTo: _authService.user!.uid)
        .snapshots();
  }

  Future<bool> checkChatExists(String uid1, String uid2) async {
    String chatID = generateChatID(uid1: uid1, uid2: uid2);
    final result = await _chatsCollection?.doc(chatID).get();
    return result?.exists ?? false;
  }

  Future<void> createNewChat(String uid1, String uid2) async {
    String chatID = generateChatID(uid1: uid1, uid2: uid2);
    final docRef = _chatsCollection!.doc(chatID);
    final chat = Chat(id: chatID, participants: [uid1, uid2], messages: []);
    await docRef.set(chat);
  }

  Future<void> sendChatMessage(
      String uid1, String uid2, Message message) async {
    String chatID = generateChatID(uid1: uid1, uid2: uid2);
    final docRef = _chatsCollection!.doc(chatID);
    await docRef.update({
      "messages": FieldValue.arrayUnion([message.toJson()]),
    });
  }

  /// ✅ Update message status to "delivered" when receiver opens the chat
  Future<void> updateMessageStatusToDelivered(String chatID) async {
    await _updateMessageStatus(
        chatID, MessageStatus.sent, MessageStatus.delivered);
  }

  /// ✅ Update message status to "seen" when receiver views the message
  Future<void> updateMessageStatusToSeen(
      String chatID, String receiverID) async {
    await _updateMessageStatus(
        chatID, MessageStatus.delivered, MessageStatus.seen, receiverID);
  }

  /// ✅ Generic function to update message status
  Future<void> _updateMessageStatus(
      String chatID, MessageStatus fromStatus, MessageStatus toStatus,
      [String? receiverID]) async {
    final docRef = _chatsCollection!.doc(chatID);
    final snapshot = await docRef.get();

    if (snapshot.exists && snapshot.data() != null) {
      var data = snapshot.data() as Map<String, dynamic>;
      List<Message> messages = (data["messages"] as List<dynamic>?)
              ?.map((e) => Message.fromJson(e as Map<String, dynamic>))
              .toList() ??
          [];

      for (var message in messages) {
        if ((receiverID == null || message.senderID != receiverID) &&
            message.status == fromStatus) {
          message.status = toStatus;
        }
      }

      await docRef.update({
        "messages": messages.map((e) => e.toJson()).toList(),
      });
    }
  }

  Stream<DocumentSnapshot<Chat>> getChatData(String uid1, String uid2) {
    String chatID = generateChatID(uid1: uid1, uid2: uid2);
    return _chatsCollection!.doc(chatID).snapshots();
  }

  Future<void> updateTypingStatus(
      String senderId, String receiverId, bool isTyping) async {
    try {
      String chatID = generateChatID(uid1: senderId, uid2: receiverId);
      final docRef = _chatsCollection?.doc(chatID);
      if (docRef != null) {
        await docRef.update({
          "typingStatus.$senderId": isTyping,
        });
      }
    } catch (e) {
      print("Error updating typing status: $e");
    }
  }

  /// ✅ Listen to other user's typing status
  Stream<bool> getTypingStatus(String senderId, String receiverId) {
    String chatID = generateChatID(uid1: senderId, uid2: receiverId);
    return _chatsCollection!.doc(chatID).snapshots().map((snapshot) {
      if (snapshot.exists && snapshot.data() != null) {
        Map<String, dynamic>? data = snapshot.data() as Map<String, dynamic>?;
        return data?["typingStatus"]?[receiverId] ?? false;
      }
      return false;
    });
  }
}
